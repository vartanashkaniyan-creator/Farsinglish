/**
 * @file migration_steps_pro_fixed.js
 * @description مدیریت پیشرفته migration دیتابیس‌ها با رعایت اصول آلفا
 * @author
 */
...
export class MigrationManager extends EventEmitter {
  ...
  add_migration_step(step, db_key) {
    ...
    if (!this.#steps[db_key]) this.#steps[db_key] = [];
    this.#steps[db_key].push(this.#clone_step(step));
  ...
  async migrate_to_version_async(db_key, target_version) {
    if (!db_key || !this.#steps[db_key]) throw new Error(`Invalid db_key: ${db_key}`);
    if (target_version > this.#steps[db_key].length) throw new Error('target_version out of range');

    const steps = this.#steps[db_key];
  ...
  async rollback_last_step_async(db_key) {
    const history_stack = this.#history[db_key];
  ...
  async migrate_with_transaction(db_key, target_version) {
    ...
    const backup_history = structuredClone(this.#history);
  ...
  get_snapshot_state(db_key) {
    ...
    return {
      steps: structuredClone(this.#steps[db_key]),
      history: structuredClone(this.#history[db_key]),
